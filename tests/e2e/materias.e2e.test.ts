...
describe('materias e conteudos', () => {
  ...
  describe('GET /api/materias', () => {
    it('por padrao esconde materias inativas; ?incluirInativas=true traz todas', async () => {
      ...
      const padrao = await app.request('/api/materias', { headers: authHeader(cookie) }, testEnv)
  ...
  describe('POST /api/materias', () => {
    it('admin cria materia', async () => {
      ...
      const response = await app.request(
        '/api/materias',
        { method: 'POST', headers: { ...jsonHeaders, ...authHeader(cookie) }, body: JSON.stringify({ nome: 'Matematica' }) },
        testEnv,
      )
    ...
    it('professor nao pode criar materia -> 403', async () => {
      ...
      const response = await app.request(
        '/api/materias',
        { method: 'POST', headers: { ...jsonHeaders, ...authHeader(cookie) }, body: JSON.stringify({ nome: 'X' }) },
        testEnv,
      )
